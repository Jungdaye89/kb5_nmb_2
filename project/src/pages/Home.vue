<template>
    <div id="header">
        <nav class="navbar navbar-expand-md mt-2 p-4" style="background-color: #ffc000">
            <span class="navbar-brand"><b>H.A</b></span>
            <div :class="navClass" class="ms-auto d-flex align-items-center">
                <img class="rounded" src="../components/data/KARINA.jpg" width="40" height="40" />
                <span class="navbar-text">
                    <router-link class="nav-link" to="/Profile">&nbsp&nbsp {{ profileStore.name }} 님 환영합니다.</router-link>
                </span>
            </div>
        </nav>
    </div>
    <div id="home" class="homecon container text-center py-5">
        ...
        <div class="d-flex justify-content-around mb-5">
            <!-- 각 동그라미에 텍스트 추가 -->
            <div class="d-flex justify-content-center align-items-center rounded-circle circle" @mouseover="onHover(index)" @mouseleave="onLeave">
                <router-link to="/RecentReport" class="text-decoration-none text-dark"> 거래내역 </router-link>
            </div>
            <div class="d-flex justify-content-center align-items-center rounded-circle circle" @mouseover="onHover(index)" @mouseleave="onLeave">
                <router-link to="/MonthlyReport" class="text-decoration-none text-dark"> 월별요약 </router-link>
            </div>
            <div class="d-flex justify-content-center align-items-center rounded-circle circle" @mouseover="onHover(index)" @mouseleave="onLeave">
                <router-link to="/Cash" class="text-decoration-none text-dark"> 가계부 쓰기 </router-link>
            </div>
        </div>
        <!-- 구분선 추가 -->
        <div class="separator mt-5"></div>
        <!-- 카드 섹션 추가 -->
        <div class="d-flex justify-content-around flex-wrap my-4 mt-5">
            <!-- 총 수입 카드 -->
...
watch(
    ...
    (newValue) => {
        ...
        newValue.forEach((item) => {
            summary.totalExpense += parseInt(item.expense) || 0;
            summary.totalIncome += parseInt(item.income) || 0;
        });
...
const chartData = computed(() => {
    ...
    for (let month in monthlySummary.value) {
        labels.push(`${month}월`);
        expenses.push(monthlySummary.value[month].totalExpense);
    }
    return {
        labels: labels,
        datasets: [
